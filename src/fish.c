...
init_fish(void)
{
    ...
    for (uint8_t i = 0; i < MAX_FISH; i++)
    {
        ...
        fish->movement_timer =
            (rand() % (MAX_MOVE_INTERVAL - MIN_MOVE_INTERVAL + 1)) + MIN_MOVE_INTERVAL; // Random interval

        // Use metasprite for the fish sprite
        move_metasprite(guppy_sprite_metasprites[0], FISH_TILE_INDEX, fish->sprite_id, fish->x,
                        fish->y); // guppy_metasprites[0] is the first frame
    }
}
